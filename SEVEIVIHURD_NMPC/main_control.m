...
Nw0         = 0;
NwV0        = 0;
Model_0     = [Psi_dado(1),N0*[S0,E0,V0,Ev0,Ia0,Is0,Iav0,Isv0,H0,U0,R0,Rv0,D0,Nw0,NwV0]];
...
E_ho=[E_ho;y(2:end,3)];
V_ho=[V_ho;y(2:end,4)];
Ev_ho=[Ev_ho;y(2:end,5)];
Ia_ho=[Ia_ho;y(2:end,6)];
Is_ho=[Is_ho;y(2:end,7)];
Iav_ho=[Iav_ho;y(2:end,8)];
Isv_ho=[Isv_ho;y(2:end,9)];
H_ho=[H_ho;y(2:end,10)];
U_ho=[U_ho;y(2:end,11)];
R_ho=[R_ho;y(2:end,12)];
Rv_ho=[Rv_ho;y(2:end,13)];
D_ho=[D_ho;y(2:end,14)];
Nw_ho=[Nw_ho;y(2:end,15)];
NwV_ho=[NwV_ho;y(2:end,16)];

N0_ho=N0_ho-D_ho(end);
Model_0_ho = [Psic_ho(end),S_ho(end),E_ho(end),V_ho(end),Ev_ho(end),Ia_ho(end),Is_ho(end),Iav_ho(end),Isv_ho(end),H_ho(end),U_ho(end),R_ho(end),Rv_ho(end),D_ho(end),Nw_ho(end),NwV_ho(end)];
costd=inf;
end
...
E_lo=[E_lo;y(2:end,3)];
V_lo=[V_lo;y(2:end,4)];
Ev_lo=[Ev_lo;y(2:end,5)];
Ia_lo=[Ia_lo;y(2:end,6)];
Is_lo=[Is_lo;y(2:end,7)];
Iav_lo=[Iav_lo;y(2:end,8)];
Isv_lo=[Isv_lo;y(2:end,9)];
H_lo=[H_lo;y(2:end,10)];
U_lo=[U_lo;y(2:end,11)];
R_lo=[R_lo;y(2:end,12)];
Rv_lo=[Rv_lo;y(2:end,13)];
D_lo=[D_lo;y(2:end,14)];
Nw_lo=[Nw_lo;y(2:end,15)];
NwV_lo=[NwV_lo;y(2:end,16)];

N0_lo=N0_lo-D_lo(end);
Model_0_lo = [Psic_lo(end),S_lo(end),E_lo(end),V_lo(end),Ev_lo(end),Ia_lo(end),Is_lo(end),Iav_lo(end),Isv_lo(end),H_lo(end),U_lo(end),R_lo(end),Rv_lo(end),D_lo(end),Nw_lo(end),NwV_lo(end)];
costd=inf;
end
...
fill([tplotsbeforemodel(npd) tplotsbeforemodel(npts) tplotsbeforemodel(npts) tplotsbeforemodel(npd)], [0 0 2e3 2e3], [0.87, 0.87, 0.87],'FaceAlpha', 0.5, 'EdgeColor','w');
grid off; hold on;
plot(tplotsbeforemodel,NwVm,'Color',[0, 0.4470, 0.7410],'LineStyle','--','LineWidth',3); 
plot(tplotsbeforemodel,NwV_ho,'Color',[0.9300, 0.4350, 0.0980],'LineWidth',3); 
plot(tplotsbeforemodel,NwV_lo,'Color',[0.4660, 0.6740, 0.1880],'LineWidth',3); 
plot(tplotsDATA,V_dado,'k.','MarkerSize',10); 
set(gca,'FontSize',20,'LineWidth',2,'FontWeight','Bold')
ylabel('Vaccinated')
title('(V)', 'FontSize', 20);
xlim([tplotsbeforemodel(1) tplotsbeforemodel(end)])
