...
Nw0         = 0;
NwV0        = 0;
Model_0     = N0*[S0,E0,V0,Ev0,Ia0,Is0,Iav0,Isv0,H0,U0,R0,Rv0,D0,Nw0,NwV0];
...
for i = 1:t_opt:tf
    ...
    Dados    = [Cases_dado(i:i+t_opt),H_dado(i:i+t_opt),U_dado(i:i+t_opt),D_dado(i:i+t_opt)];%,V_dado(i:i+t_opt)];
    
    parameters=covidParameters(Dados,Model_0,N0,t,Psif);
      
    x(:,l) = parameters;
    
    [t_m,S_m,E_m,V_m,Ev_m,Ia_m,Is_m,Iav_m,Isv_m,H_m,U_m,R_m,Rv_m,D_m,Nw_m,NwV_m]=solver_model(x(:,l),t,Model_0,N0,Psif);
    ...
    Nw = [Nw;Nw_m(2:end)];  
    NwV = [NwV;NwV_m(2:end)];  
    Model = [S,E,V,Ev,Ia,Is,Iav,Isv,H,U,R,Rv,D,Nw,NwV];
    ...
    Model_0         = [S(end),E(end),V(end),Ev(end),Ia(end),Is(end),Iav(end),Isv(end),H(end),U(end),R(end),Rv(end),D(end),Nw(end),NwV(end)];
end
...
[t_m,S_m,E_m,V_m,Ev_m,Ia_m,Is_m,Iav_m,Isv_m,H_m,U_m,R_m,Rv_m,D_m,Nw_m,NwV_m]=solver_model(x(:,end),tf:1:tf+t_pred,Model_0,N0,Psif);

t_p(tf+1:tf+t_pred) = t_m(2:end);
S(tf+1:tf+t_pred) = S_m(2:end); 
E(tf+1:tf+t_pred) = E_m(2:end);
V(tf+1:tf+t_pred) = V_m(2:end);
Ev(tf+1:tf+t_pred) = Ev_m(2:end);
Ia(tf+1:tf+t_pred) = Ia_m(2:end); 
Is(tf+1:tf+t_pred) = Is_m(2:end);
Iav(tf+1:tf+t_pred) = Iav_m(2:end); 
Isv(tf+1:tf+t_pred) = Isv_m(2:end);
H(tf+1:tf+t_pred) = H_m(2:end); 
U(tf+1:tf+t_pred) = U_m(2:end); 
R(tf+1:tf+t_pred) = R_m(2:end); 
Rv(tf+1:tf+t_pred) = Rv_m(2:end); 
D(tf+1:tf+t_pred) = D_m(2:end);
Nw(tf+1:tf+t_pred) = Nw_m(2:end);
NwV(tf+1:tf+t_pred) = NwV_m(2:end);
...
V_min = V*0.95;
V_max = V*1.05;
shadedplot(t_p,V_min',V_max',[0.9 0.9 0.95],[0.9 0.9 0.95]);
hold on
p3=plot(t_p,NwV,'b','LineWidth',3);
